...
const displayMovements = function (movements, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    let dateLabel = `${String(day).padStart(2, 0)}/${String(month).padStart(
      2,
      0
    )}/${year}`;
...
labelDate.textContent = `${day}/${month}/${year}   ${String(hour).padStart(
  2,
  0
...
const logOut = function () {
  time = 2 * 60;
  const tick = function () {
    const [min, sec] = [Math.trunc(time / 60), time % 60];
